...
const ProductionCard: FC<ProductionCardProps> = ({ image }) => {
    ...
    return (
        <Link href={'https://playpla.co.kr/?idx=25'}>
            <div className="w-[140px] h-[200px] bg-white rounded-xl">
                <div className="w-full h-[121px] bg-[#464646] rounded-t-xl flex justify-center items-center relative">
                    <div
                        className="w-[52px] py-[2px] bg-[#C24340] flex justify-center items-center rounded-full absolute top-[10px] left-[15px]">
                    ...
                    >
                        {isLiked ? <FaHeart className="text-red-500" size={20}/> :
                            <FaRegHeart className="text-white" size={20}/>}
                    </div>
                    <Image src={image} alt="prod-img" width={100} height={100}/>
                </div>
